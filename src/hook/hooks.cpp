...
namespace Hook
{
   ...
   template <typename T, typename... Args> void LockedCall(std::atomic<bool> &flag, T &func, Args &&...args)
   {
      flag = true;
      func(args...);
      flag = false;
   ...
   template <auto T> void InjectTTFChar(unsigned char symbol, int x, int y)
   {
   ...
   void __fastcall HOOK(addst_top)(graphicst_ *gps, std::string &str, __int64 a3)
   {
      if (gps && !str.empty() && Config::Setting::enable_translation && !ttf_injection_lock)
      {
         ...
         auto translation = Dictionary::GetSingleton()->GetMulti(str.c_str(), gps->screenx, gps->screeny, true);
         if (translation)
         {
